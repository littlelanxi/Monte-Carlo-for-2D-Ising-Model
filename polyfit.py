...
def generate_xLyL(kappa,nu):
    ...
    for index in range(N):
        ...
        if abs(x1)<=const:   
            xL[3*index]=x1
            c1=Q_list_8[index]
            yL[3*index]=c1*L1**(-kappa/nu)
        if abs(x2)<=const:   
            xL[3*index+1]=x2
            c2=Q_list_12[index]
            yL[3*index+1]=c2*L2**(-kappa/nu)
        if abs(x3)<=const:   
            xL[3*index+2]=x3
            c3=Q_list_16[index]
            yL[3*index+2]=c3*L3**(-kappa/nu)
    return xL,yL
...
kappa_list=[]
nu_list=[]
for kappa in np.arange(1.5,2,0.01):
    for nu in np.arange(0.8,1.2,0.01):
        tL_array,QL_array=generate_xLyL(kappa,nu)
